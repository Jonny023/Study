...
//    new File(dir, className + ".java").withPrintWriter { out -> genEntity(out, className, fields,table) }
    ...
    def fields = calcFields(table)
    // 忽略的字段，这些字段定义在父类里面抽取为公共属性
    def IGNORE_FIELDS = ["id", "create_time", "update_time", "create_user", "update_user"]
    fields = fields.findAll { !IGNORE_FIELDS.contains(it.name) }
...
def genRepository(out, table, className, fields, basePackage, entity, repository) {
    ...
    out.println "public interface ${className}Repository extends BaseRepository<$className, Long> {\n\n}"
}

def genEntity(out, table, className, fields, basePackage, entity) {
    out.println "package ${basePackage}.${entity};"
    out.println ""
    out.println "import cn.com.geely.jpa.BaseEntity;"
