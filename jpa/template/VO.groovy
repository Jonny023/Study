...
//    new File(dir, className + ".java").withPrintWriter { out -> genEntity(out, className, fields,table) }
    ...
    //生成JPA
    PrintWriter printWriter = new PrintWriter(new OutputStreamWriter(new FileOutputStream(new File(entityPath, className + "VO.java")), "UTF-8"))
    //编码格式
...
//        }
        ...
        if (isNotEmpty(it.comment)) {
            ...
            out.println "\t@ApiModelProperty(value = \"${it.property}\")"
        }
